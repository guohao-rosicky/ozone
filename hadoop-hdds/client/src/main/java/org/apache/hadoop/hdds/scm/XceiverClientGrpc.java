...
public class XceiverClientGrpc extends XceiverClientSpi {
   ...
   * @param pool - a {@link XceiverClientGrpcConnectionPool} XceiverClientGrpc Connection Pool.
   */
   ...
   */
  public XceiverClientGrpc(Pipeline pipeline, ConfigurationSource config) {
    this(pipeline, config, new XceiverClientGrpcConnectionPool(config, null));
  }
  ...
  private synchronized void connectToDatanode(DatanodeDetails dn)
    ...
    connects.put(dn.getUuid(), connectFromPool(dn));
  }

  protected XceiverClientGrpcConnectionPool.Connection connectFromPool(
      DatanodeDetails dn) throws IOException {
    return connectPool.connect(dn);
  }

  public XceiverClientGrpcConnectionPool getConnectPool() {
    return connectPool;
  }
  ...
  public synchronized void close() {
    closed = true;
    connects.forEach((k, v) -> connectPool.close(v));
  }
  ...
  public XceiverClientReply sendCommandAsync(
    ...
    XceiverClientProtocolServiceStub asyncStub =
        connects.get(dnId).getAsyncStub();
